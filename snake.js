    ...
    setTimeout(function () {
        ...
        if (gameplay === 1) {
            ...
            if (didEatFood) {
                ...
                const music = new Audio('SnakeGame_power.wav');
                music.play();
                // music.loop =true;
...
function gameover1() {
    ...
    if (snake[0].x >= can.width || snake[0].y >= canvas.height || snake[0].x === 0 || snake[0].y === 0) {
        ...
        ctx.drawImage(gameover, can.width / 2, can.height / 4, 100, 100);
        ctx.font = "14px Arial";
        ctx.fillStyle = "white";
        ctx.textAlign = "center";
        ctx.fillText("Game Over", can.width / 2, can.height / 10);
        gameplay = 0;
        const music = new Audio('SnakeGame_gameover.wav');
        music.play();
      
        //alert("gameover");
        var para = new URLSearchParams();
        para.append("KEY", plyerscore);
       
        setTimeout(() => {  console.log("World!"); location.href = "end.html?" + para.toString(); }, 2000);
